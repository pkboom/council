<template>
    <div>
        <div class="my-10 p-4 rounded border border-grey-light flex justify-between" v-for="(user, key) in leaderboard" :key="user.id">
            <div class="flex">
                ...
                <div>
                    ...
                    <span class="inline px-2 bg-green rounded font-semibold text-white">{{ user.reputation }}</span>
                </div>
            </div>
...
<script>
    export default {
        ...
        methods: {
            fetch() {
                axios.get('api/leaderboard').then(this.refresh);
            },
